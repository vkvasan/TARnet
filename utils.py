...
def preprocess(prop, X_train, y_train, X_test, y_test):
    ...
    '''
    if prop['task_type'] == 'classification':
        y_train_task = torch.as_tensor(y_train, dtype=torch.long)
...
def preprocess_nb(prop, X_train, y_train, X_test, y_test):
    mean, std = mean_standardize_fit(X_train)
    X_train, X_test = mean_standardize_transform(X_train, mean, std), mean_standardize_transform(X_test, mean, std)
...
def initialize_training_nb(config):
    model =  multitask_transformer_class.MultitaskTransformerModel(config.task_type, config.device, config.nclasses, config.seq_len, config.batch, \
    ...
    best_model =  multitask_transformer_class.MultitaskTransformerModel(config.task_type, config.device, config.nclasses, config.seq_len, config.batch, \
    config.input_size, config.emb_size, config.nhead, config.nhid, config.nhid_tar, config.nhid_task, config.nlayers, config.dropout)
    ...
    criterion_tar = torch.nn.MSELoss()
    criterion_task = torch.nn.CrossEntropyLoss() if config.task_type == 'classification' else torch.nn.MSELoss() # nn.L1Loss() for MAE
    optimizer = torch.optim.Adam(model.parameters(), lr = config.lr)
    best_optimizer = torch.optim.Adam(best_model.parameters(), lr = config.lr) # get new optimiser
...
def compute_task_loss(nclasses, model, device, criterion_task, y_train_task, batched_input_task, task_type, num_inst, start):
    ...
    #print( out_task[:num_inst].shape)
    #print(torch.as_tensor(y_train_task[start:start + num_inst]).shape)
...
def multitask_train_nb(model, criterion_tar, criterion_task, optimizer, X_train_tar, X_train_task, y_train_tar_masked, y_train_tar_unmasked, \
...
def training_nb(model, optimizer, criterion_tar, criterion_task, best_model, best_optimizer, X_train_task, y_train_task, X_test, y_test, prop):
    tar_loss_masked_arr, tar_loss_unmasked_arr, tar_loss_arr, task_loss_arr, min_task_loss = [], [], [], [], math.inf
    acc, rmse, mae = 0, math.inf, math.inf
    torch.cuda.empty_cache()
    instance_weights = torch.as_tensor(torch.rand(X_train_task.shape[0], prop.seq_len), device = prop.device)
    for epoch in range(1, prop.epochs + 1):
        ...
        tar_loss_masked, tar_loss_unmasked, task_loss, instance_weights = multitask_train_nb(model, criterion_tar, criterion_task, optimizer, 
                                            X_train_tar, X_train_task, y_train_tar_masked, y_train_tar_unmasked, y_train_task, 
                                            boolean_indices_masked, boolean_indices_unmasked, prop)
        ...
        # Saved best model state at the lowest training loss is evaluated on the official test set
        test_metrics,avg_inference_time = test(best_model, X_test, y_test, prop.batch, prop.nclasses, criterion_task, prop.task_type, prop.device, prop.avg)
        
        print("Evaluation Results:", test_metrics)
        print("Average Inference Time per Batch:", avg_inference_time, "seconds")
        #prop.task_type = prop.task_type.to(prop.device)
        if prop.task_type == 'classification' and test_metrics[1] > acc:
            acc = test_metrics[1]
        elif prop.task_type == 'regression' and test_metrics[0] < rmse:
            rmse = test_metrics[0]
